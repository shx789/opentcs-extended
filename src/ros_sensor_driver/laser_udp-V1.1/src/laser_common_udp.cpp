...
ClaserCommonUdp::ClaserCommonUdp(const std::string &hostname, const std::string &port, int &timelimit, 
                                    CParserBase *parser, ros::NodeHandle &nh, ros::NodeHandle &pnh) :
    ClaserCommon(parser, nh, pnh),
    mHostName(hostname),
    mPort(port),
...
ClaserCommonUdp::~ClaserCommonUdp()
{
    StopScanner();
    CloseDevice();
}

int ClaserCommonUdp::InitDevice()
{
    int opt = 1;
...
int ClaserCommonUdp::CloseDevice()
{
    if(mSocket != -1)
    {
        close(mSocket);
...
int ClaserCommonUdp::SendUdpData2Device(char *buf, int length)
{
    int n = -1;
    if(mSocket > 0)
    {
...
int ClaserCommonUdp::SendDeviceReq(const char *req, std::vector<unsigned char> *resp)
{
    if(mSocket == -1)
    {
        ROS_ERROR("SendDeviceReq: Socket NOT open");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "laser_ - SendDeviceReq: socket NOT open!");
    ...
    if(SendUdpData2Device((char *)req, SIZE_OF_CMD) != SIZE_OF_CMD)
    {
        ...
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "laser_ - SendDeviceReq: Write command failed!");
...
int ClaserCommonUdp::GetDataGram(unsigned char *receiveBuffer, int bufferSize, int *length)
{
    int len;
    struct timeval tv;
    socklen_t addrlen;
    ...
    if(mSocket == -1)
    {
        ...
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "laser_ - GetDataGram: socket NOT open!");
    ...
    if(1)//(!stream_stopped_)
    {
        ...
        {
            std::string errorStr =  "laser_ - GetDataGram: No full response for read after " + mTimeLimit;
            errorStr = errorStr + "S";
