...
int main(int argc,char **argv)
{
   ...
   n.param<std::string>("bms_port",bms_port,"/dev/ttyUSB0");
   signal(SIGINT, mySigIntHandler);  											//把原来ctrl+c中断函数覆盖掉，把信号槽连接到mySigIntHandler保证关闭节点

   timer1 = nh.createTimer(ros::Duration (1), timercallback,false,true);    //设置定时器每隔1s发送数据
   ...
   try
	...
	return -1;
	    ...
	    int byteRead,rest_len,pack_len;
	    //printf("size:%d\n",n);
	    byteRead = ser.read(buffer,n);
	    rest_len =byteRead;
	    ...
	    if(byteRead >=13)
	    {
            ...
            while (1)
            {
                ...
                for(;i<rest_len;i++)
                {
                    if(buffer[i] ==0xA5 )
                    {
                        for(u8 i =0;i<sizeof(return_robot_data.rx_buffer);i++)
                        {
		    ...
		    //printf("1\n");	
                    ...
                    soc = (return_robot_data.rx_buffer[10]<<8) | return_robot_data.rx_buffer[11];
                    current = current_check - 30000;
                }
                 else if(return_robot_data.rx_buffer[0] == 0xA5 && return_robot_data.rx_buffer[2] == 0x93 &&  return_robot_data.rx_buffer[12] == check )
                {
                     //printf("2\n");
                ...
                else if(return_robot_data.rx_buffer[0] == 0xA5 && return_robot_data.rx_buffer[2] == 0x96 &&  return_robot_data.rx_buffer[12] == check )
                {
                     //printf("3\n");            
                ...
                else if(return_robot_data.rx_buffer[0] == 0xA5 && return_robot_data.rx_buffer[2] == 0x98 &&  return_robot_data.rx_buffer[12] == check )
                {
                    //printf("4\n");
