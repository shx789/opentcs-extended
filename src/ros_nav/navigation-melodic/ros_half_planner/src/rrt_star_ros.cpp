...
namespace RRTstar_planner
{
  ...
  {}
   ...
   RRTstarPlannerROS::RRTstarPlannerROS(std::shared_ptr<costmap_2d::Costmap2D>  &costmap_ros) :
    initialized_(false)
  {
  ...
  //变量初始化
  void RRTstarPlannerROS::initialize(std::shared_ptr<costmap_2d::Costmap2D>  &costmap_ros)
  {
    ...
    if (!initialized_)
    {
      ...
      plan_pub_ = private_nh.advertise<nav_msgs::Path>("rrt_plan",1); //发布全局计算
      marker_pub_ = private_nh.advertise<visualization_msgs::Marker>( "rrt_visualization_marker", 0 ); //发布可视化扩展过程
      ...
      //路径优化参数
      path_point_spacing_ = 0.2;//0.025; //路径点间隔
      angle_difference_ = M_PI/3;//M_PI/20; //前后相邻点向量角度差
