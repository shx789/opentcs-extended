        ...
        first_reconfigure_call_(true)
{
  ...
  //add zxp
  amcl_status_pub = nh_.advertise<std_msgs::Float32>("amcl_status", 2, true);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(lasers_update_[laser_index])
  {
    ...
    msg.data = Amcl_status.total;
    if(Amcl_status.have_new == 1)
    {
       amcl_status_pub.publish(msg);
       Amcl_status.have_new = 0;
    }
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      if (tf_broadcast_ == true)
      {
        ...
        //add zxp 20220409
        tmp_tf_stamped.child_frame_id = "amcl_odom";//odom_frame_id_;
