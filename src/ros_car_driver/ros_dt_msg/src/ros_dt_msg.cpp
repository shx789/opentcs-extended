...
void lift_table_control(u8 data)
{
    dt_Litf_Table.prot.Header = HEADER;
    dt_Litf_Table.prot.Len = 0x0A;
    dt_Litf_Table.prot.Type = 0x02;
    dt_Litf_Table.prot.Cmd = 0x88;
    dt_Litf_Table.prot.Num = 0x01;
    dt_Litf_Table.prot.Data = data;
    dt_Litf_Table.prot.Check = 0;
    for (int i = 0; i < dt_Litf_Table.prot.Len - 2; i++)
    {
        dt_Litf_Table.prot.Check += dt_Litf_Table.data[i];
    }
    ser.write(dt_Litf_Table.data, sizeof(dt_Litf_Table.data));
}
...
void dt_control_callback(const geometry_msgs::Twist::ConstPtr &twist_aux)
{
    ...
    {
        //dt_control2(control_msg->lspeed, control_msg->rspeed);
    }
}
...
void dt_lift_table_callback(const std_msgs::UInt8::ConstPtr status)
{
    lift_table_control(status->data);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber dt_control_sub = nh.subscribe("/cmd_vel", 100, dt_control_callback);
     ros::Subscriber dt_lift_table_sub = nh.subscribe("/lift_table", 100, dt_lift_table_callback);                   // 软件急停 0为不急停，1为急停
    ...
    while (ros::ok())
    {
        ...
        {
            ser.read(buffer, sizeof(RXRobotData20MS.data));
            ...
            if (RXRobotData20MS.prot.Header == HEADER && RXRobotData20MS.prot.Check == TempCheck && RXRobotData20MS.prot.Cmd == 0x81)
            {
                ...
                if (control_mode == 1)
                {
                    ...
                    dt1_msg.Light12 = RXMode1.prot.Light12;
                    dt1_msg.Light34 = RXMode1.prot.Light34;
